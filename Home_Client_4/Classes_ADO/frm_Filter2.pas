...
uses
  ...
  Dialogs, DB, DBLogDlg, StdCtrls, Grids, DBGridEh, FormBaseV1_1,
   ExtCtrls, ComCtrls,teEngine,mxcommon,comObj, DBCtrls,Excel97, DBGridEhImpExp,
  Buttons, ADODB;

type
  TForm_Filter2 = class(TForm)
...
//        Открытие формы
//---------------------------------------------------------------------
procedure TForm_Filter2.FormShow(Sender: TObject);
var
   i,J:integer;
...
begin
   ...
   //endif
   //-------------------------------------------------------Сессия
   QUERY_TMP.connection:=PARENT_FORM.OraQuery1.connection;
...
//---------------------------------------------------------------------
procedure TForm_Filter2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   QUERY_TMP.close;
end;
...
//---------------------------------------------------------------------
procedure TForm_Filter2.UPDSTATE(Sender: TObject);
VAR
   i,j:integer;
   RCH:TCheckBox;
...
//---------------------------------------------------------------------
procedure TForm_Filter2.Button3Click(Sender: TObject);
VAR
      ...
      begin
         ...
         if (PARENT_FORM.mainfld_types[N_ACT_ELEMENT]<>ftDateTime) then
            ...
            //--------------------Иначе
               TX_VALUE[N_ACT_ELEMENT].Text:=
               TX_VALUE[N_ACT_ELEMENT].Text+
               floattostr(strtofloat(vartostr(LISTBOX1.Items[n_sel_item])));
            //endif
         //endif
...
//---------------------------------------------------------------
procedure TForm_Filter2.Button4Click(Sender: TObject);
var
   go_filter:string;
   i:integer;
   DateFormStr:string;
            ...
            begin
               if PARENT_FORM.mainfld_types[i]=ftDateTime then
                  if COM_FIL_FLD[i].text='BETWEEN' then
                     ...
                     COM_FIL_FLD[i].text+
                     ' CDate('+CHR(39)+DATETOSTR(DATESTART_FLD[i].Date)+CHR(39)+')'+
                     ' and CDate('+CHR(39)+DATETOSTR(DATEEND_FLD[i].Date)+CHR(39)+')'
                  else
                     go_filter:=' and '+PARENT_FORM.mainfld_go[i]+' '+
                     COM_FIL_FLD[i].text+
                     ' CDate('+CHR(39)+DATETOSTR(DATESTART_FLD[i].Date)+CHR(39)+')'
...
//---------------------------------------------------------------------
procedure TForm_Filter2.UPDSTATE1(Sender: TObject);
VAR
   RCH:TEdit;
   intag:integer;
...
//             Создание формы
//---------------------------------------------------------------------
procedure TForm_Filter2.FormCreate(Sender: TObject);
var
   i:integer;
...
begin
   for i:=1 to 50 do
      begin
         //------------------------Названия полей
         LAB_FIL_FLD[i]:=TLabel.Create(sender as TForm_Filter2);
         ...
         //---------------------------Тексты фильтров
         TX_VALUE[i]:=TEdit.Create(sender as TForm_Filter2);
         ...
         DATESTART_FLD[i]:=TDateTimePicker.Create(Sender as TForm_Filter2);
         ...
         DATEEND_FLD[i]:=TDateTimePicker.Create(Sender as TForm_Filter2);
         ...
         //-----------------------------Тип условия
         COM_FIL_FLD[i]:=TComboBox.Create(sender as TForm_Filter2);
         ...
         //--------------------------------Чеки выбора условий
         CHK_FIL_FLD[i]:=TCheckBox.Create(sender as TForm_Filter2);
...
procedure TForm_Filter2.SpeedButton1Click(Sender: TObject);
begin
