...
uses
  ...
  Dialogs, DB, StdCtrls, Mask, DBCtrlsEh, DBGridEh,
  DBLookupEh, Buttons, ADODB;
...
type
  ...
  public
    ...
    lookup_oraq:array[1..10] of TADOQuery;
       ...
       begin
          ...
          if TMP_TYPEFLD=1001 then
             begin
                ...
                TADOquery.create(sender as TFormUNIEDIT);

                lookup_oraq[nlookupfields].connection:=
                ORAQUERY1.connection;

                lookup_oraq[nlookupfields].SQL.Text:=
                'select  B.'+filt_DOPID+', B.'+filt_NAME+
                ' AS DOP_TX from '+filt_tab+' B order by 2';
...
begin
      ...
      begin
         if vartostr(num_fields[i].Value)<>'' then
            begin
               ...
               //------------------INS2
               INS_SUBSTR2:=INS_SUBSTR2+
               floattostr(strtofloat(vartostr(num_fields[i].Value)));
   ...
   for i:=1 to ndatetimefields do
      begin
         ...
         //------------------INS2
         INS_SUBSTR2:=INS_SUBSTR2+
         'Cdate('+CHR(39)+VARTOSTR(date_fields[i].Value)+CHR(39)+')';
