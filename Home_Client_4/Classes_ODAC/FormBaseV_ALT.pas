...
uses
  ...
  MemDS, DBAccess, Ora, DBCtrlsEh, DBLookupEh, Grids, DBGridEh, OraSmart,
...
type
  TFormBaseV_ALT = class(TForm)
    ...
    procedure FORM_MAININIT(GLSESSION:TOraSession;Sender: TObject);virtual;
  ...
  public
    ...
    //-------------------------------Запросы фильтров
    oraqgo:array[1..15] of TOraquery;
    ...
    xlSHEET:string;
    xlFILE:string;
    ENGINE_TYPE:integer;  // 1- COPY  3- ODBC
    IS_Visible:boolean
    );
    ...
    //-----------------------------------------------------------
    procedure EXCEL_SCREEN(SQL_IN:widestring);
...
begin
   ...
   if aggreg=false then
      begin
         if IS_ROWID=TRUE THEN
            //--------------------Запрашивать RowID
            TMP_SQL:='SELECT '+viewflds+
            ',A.RowID  FROM '+TABGO+' A '+basestr+
            glfilter+actualsort
         ELSE
   ...
   IF Trim(ORAQUERY1.SQL.TEXT)<>Trim(TMP_SQL) then
      begin
         ...
         //-------------------Если источник данных таблица
         //       устанавливаем ссылку на нее как update object
         if IS_QUERY=false then
            ORAQUERY1.UpdatingTable:=TABGO
         else
...
//                       Оформление формы
//--------------------------------------------------------------------------
procedure TFormBaseV_ALT.FORM_MAININIT(GLSESSION:TOraSession;Sender: TObject);
var
   labgo:tlabel;
...
begin
    ...
    //         Оформление формы
    //---------------------------------------------------------
    oraquery1.Session:=GLSESSION;
    oraquery2.Session:=GLSESSION;
    oraquery3.Session:=GLSESSION;
    oraquery_COP.Session:=GLSESSION;
       ...
       begin
          ...
          //------------------------------------------------------
             begin
                ...
                LABFIL[nfilters].Caption:= 'select /*+ CHOOSE*/ '+filt_id+','+filt_name+
                ' DOP_TX from '+TABGO_FILTER+' A ';
                ...
                //-------------------------------- запрос фильтра
                oraqgo[nfilters]:=TOraquery.create(sender as TFormBaseV_ALT);
                oraqgo[nfilters].Session:=oraquery1.Session;
    ...
    while not(oraquery2.Eof) do
       begin
          ...
          'select /*+ CHOOSE*/ B.'+filt_DOPID+', B.'+filt_NAME+
          ' DOP_TX from '+filt_tab+' B WHERE B.'+
          filt_DOPID+' IN (SELECT '+filt_ID+' FROM '+TABGO_FILTER+' A ';
...
begin
   SCREEN.Cursor:=crHourGlass;
   EXCEL_SCREEN(oraquery1.SQL.text);
   SCREEN.CURSOR:= CRDEFAULT;
end;
...
//--------------------------------------------------------------------------
procedure  TFormBaseV_ALT.EXCEL_SCREEN(SQL_IN:widestring);
begin
   EXCEL_OUT(SQL_IN,false,false,'','',1,true);
end;
...
BEGIN
  ...
  //         Закачка данных
  //--------------------------------------------
  xltab:=xlsh.QueryTables.add(
  'ODBC;DSN='+oraquery1.Session.Server+
  ';UID='+oraquery1.Session.Username+
  ';PWD='+oraquery1.Session.Password+
  ';DBQ='+oraquery1.Session.Server+';'+
  'DBA=W;APA=T;FEN=T;QTO=T;FRC=10;FDL=10;LOB=T;'+
  'RST=T;FRL=F;MTS=F;CSR=F;PFC=10;TLO=0;',
  xlsh.RANGE['A1','A1'],
  SQL_IN);
...
var
   tmp_bkm:TBookmark;
begin
   tmp_bkm:=oraquery1.GetBookmark;
   oraquery1.Refresh;
   try
      oraquery1.GotoBookmark(tmp_bkm);
   finally
      oraquery1.FreeBookmark(tmp_bkm);
   end;
end;
