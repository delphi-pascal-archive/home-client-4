...
uses
  ...
  Dialogs, FormBaseV_ALT, DBGridEh, ADODB, Menus, OraSmart, DB, MemDS,
  DBAccess, Ora, Buttons, ExtCtrls, Grids, StdCtrls, Mask, DBCtrlsEh,
  DBLookupEh,FORMUNIEDIT1;
...
var
  FormBaseV_EDIT: TFormBaseV_EDIT;
...
//          Начальное оформление формы (Потомок)
//-----------------------------------------------------------------------
procedure TFormBaseV_EDIT.FORM_MAININIT(GLSESSION:TOraSession;Sender: TObject);
var
   i:integer;
   tek_column:integer;
...
begin
   ...
   oraquery1.SQLRefresh.ADD(
   'SELECT * FROM '+tabgo+' where '+
   tabgo_KeyEdit+'=:OLD_'+tabgo_KeyEdit);

   oraquery3.UpdatingTable:=tabgo_UpdatingTable;
  ...
  if self.n_form_edit<>0 then
     begin
        ...
        //-----------------------------------
        FORMEDITGO.OraQuery1.Session:=oraquery1.Session;
        FORMEDITGO.OraQuery2.Session:=oraquery1.Session;
        FORMEDITGO.OraQuery3.Session:=oraquery1.Session;
...
BEGIN
   ...
   if IS_ROWID=TRUE then
      oraquery3.SQL.Text:='SELECT A.rowID, A.* FROM '+
   ...
   FormRecEdit.Form_Execute(oraquery3,EditField,EditString,EditType);
   oraquery3.Close;
